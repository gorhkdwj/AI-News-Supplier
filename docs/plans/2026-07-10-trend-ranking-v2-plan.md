...
> 부기(2026-07-12, D-008→D-010): V2-0~V2-5 구현 완료. 워밍업 달력 기준일은 상시 수집이 시작된 2026-07-12이며, 워밍업 ~07-19 → shadow 비교 ~07-26 → 게이트 평가 순으로 진행합니다. 실사용자 발생으로 개선 묶음은 0.2.0으로 선배포하고(D-010), **v2 기본 전환은 게이트 통과 후 0.3.0**, legacy 제거는 0.4.0입니다. V2-7 표의 버전 표기는 이에 따라 한 단계씩 이동해 읽습니다. 상세 게이트와 버전 규범은 기준 계약 13절을 따릅니다.
