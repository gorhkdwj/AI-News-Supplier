...
- 승인 게이트 통과 후 0.2.0에서 v2 Overview를 기본으로 전환합니다. 0.2.0은 v2 전환과 함께 D-007 개선 묶음(Node 22.12 상향, 숨김 스케줄러, 업데이트 안내, 이중 언어 README, 커스텀 피드 문서)을 포함하며, 게이트 통과 전에는 publish하지 않습니다(D-008).
- legacy 실행 옵션은 한 minor release 유지하고 0.3.0에서 제거합니다. hotness 출력 별칭은 1.0까지 유지합니다.
