...
### 11.4 세션 근거 버킷별 quota (B-004)

- 광범위 토픽에서 근거 40건이 한 유형에 편중되는 것을 막습니다(실측: bm25 상위 40건이 'agent'는 레포 36건, 'model'은 논문 31건 편중, 120건으로 넓히면 전 버킷 공급 존재 — 2026-07-20).
- 자료 검색은 근거 한도(40)의 3배(120)로 1회 수행하고, 결과를 버킷(official/papers/repos/discussion)으로 나눈 뒤 다음 규칙으로 선별합니다.
  - 각 버킷에 기본 `floor(40/4)`를 버킷 내 관련도(bm25) 순으로 배정합니다.
  - 남은 자리는 official → papers → repos → discussion 순서로 공급이 남은 버킷에 한 자리씩 순환 배분하며, 40을 채우거나 공급이 소진되면 멈춥니다(10.3 Overview 배분과 동일한 설계).
- `search.matched`는 과요청 한도(120) 안에서 매칭된 자료 수이며, 근거로 실리는 수(≤40)와 다를 수 있습니다.
- from-item의 출발 항목(11.3)은 검색 순위와 무관하게 소속 버킷 맨 앞이므로 quota에서 항상 살아남습니다.
