...
- 사용자 터미널에서 `npm login` 재인증(브라우저 인증) 후 `npm publish` 재시도.
- **최종 해결(2026-07-19 23:50)**: 재인증 후 publish 성공. 레지스트리에 0.3.0 등록·latest 태그 확인, 클린 폴더 실설치로 `--version` 0.3.0·MCP 핸드셰이크(도구 9종·기본 v2 overview) 검증 완료.
...
- 2026-07-14, 문서만 변경한 커밋(64a3ed3) 푸시 후 CI의 test 잡이 3개 OS 전부 실패. 같은 코드가 07-12까지는 통과.

**증상**

- `tests/cli/trends.test.ts` 3건 중 2건 실패: JSON 배열 길이 3 기대 → 0 반환. 로컬에서도 동일 재현(코드·환경 무관).

**확인된 원인**

- fixture 날짜가 `NOW = '2026-07-10T12:00:00.000Z'`로 **고정**되어 있는데, CLI 프로세스는 실제 현재 시각으로 조회 윈도를 계산(itemStore.ts:162 `Date.now()`). community 채널 기본 윈도는 72시간(service.ts DEFAULT_HOURS) — 07-12 실행 시점엔 fixture가 48시간 이내라 통과했고, 07-14엔 약 96시간 경과로 전부 윈도 밖 → 0건. 즉 "시한폭탄 테스트": 2026-07-13 12:00 UTC 이후 무조건 실패.

**조치**

- fixture 시각을 고정 문자열 대신 실행 시점 1시간 전 상대값(`new Date(Date.now() - 3_600_000)`)으로 변경 — 최종 해결. 전체 테스트 33파일/234건 로컬 통과 확인.
- 동일 유형 전수 점검: 프로세스 spawn 테스트는 trends·mcp smoke 2개뿐이고, mcp smoke는 이미 상대 시각 사용. 고정 날짜를 쓰는 나머지 25개 파일은 모두 `now`를 주입받는 인프로세스 테스트라 안전.

**재발 방지**

- CLI를 자식 프로세스로 실행하는 테스트는 시계를 주입할 수 없으므로, fixture 타임스탬프는 반드시 실행 시점 기준 상대 시각으로 만든다. 고정 날짜 fixture는 시계를 주입받는 순수 함수 테스트에서만 사용한다.

### T-012 · design_learning_session의 근거 자료가 빈 배열로 반환됨

**발생 상황**

- Codex에서 ains MCP로 최신 트렌드 확인 후 학습 세션 설계를 요청. 지시문 템플릿은 생성됐으나 근거 자료(공식/논문/레포/커뮤니티) 4개 버킷이 전부 빈 배열. 같은 세션에서 search_news로는 관련 논문 4건이 정상 검색됨.

**증상**

- design_learning_session이 "자료 없는 뼈대"만 반환 → 에이전트가 별도 search_news 결과를 수동 결합해야 했음.

**확인된 원인**

- 자료 수집이 `searchItems(db, topic)` 단일 호출(session.ts:65) — 토픽 문자열 전체를 FTS AND 매칭(모든 토큰이 한 항목에 동시 존재해야 함)으로 검색. 실측: 영어 1토큰 20건 → 3토큰 7건으로 급감, **한국어 토픽은 0건**(코퍼스가 사실상 전부 영어 + porter 토크나이저). 에이전트가 자연어(한국어·과잉 특정) 토픽을 그대로 전달하면 0건이 구조적으로 발생. bucketEvidence는 순수 분류라 추가 필터 없음.
- 불확실점: Codex 세션에서 전달된 정확한 topic 문자열은 미확보(패턴은 재현으로 확정).

**조치**

- 원인 확정 및 0.3.0 개선 후보 등록: ① 단계적 완화 검색(AND 0건 → OR 재시도 → 토큰 축소), ② 0건 시 응답에 사유·재시도 힌트 노출(에이전트가 영어 키워드로 자가 보정 가능하게), ③ MCP 도구 설명에 "topic은 영어 키워드 권장" 명시. 수정 전까지는 문서·도구 설명으로 안내.
- **최종 해결(2026-07-14)**: 계약 11.1 신설 후 ①~③ 구현. 검색이 AND → 0건 시 OR로 완화되고, 그래도 0건이면 instructions에 사유+영어 키워드 재시도 안내를 명시. MCP 응답에 `search { mode: exact|relaxed|none, matched }` 추가. E2E 검증: "에이전트 평가" → relaxed 1건(한국어 설명 레포 매칭), 무의미 토픽 → none+안내, "ai agent evaluation benchmark" → exact 14건. 테스트 236건 통과.

**재발 방지**

- 에이전트가 자연어를 그대로 넣는 MCP 도구의 검색 파라미터는 "정확 매칭 실패 시 완화"를 기본 설계로 하고, 0건일 때 침묵하지 않고 사유를 반환한다.

---

### T-011 · 파이프에 가려진 lint 실패가 CI에서 발견됨

**발생 상황**

- 미러 파이프라인 커밋(92c1ab3) 직후 메인 CI의 lint 단계가 3-OS에서 실패. 로컬 검증에서는 통과로 보고했었음.

**증상**

- `tools/mirror-manifest.mjs`에서 `'process' is not defined`, `'console' is not defined` (no-undef) 4건.

**확인된 원인**

- ① eslint flat config에 tools/*.mjs용 Node 전역 선언이 없었음. ② 더 근본적으로, 로컬 검증 명령이 `npm run lint 2>&1 | tail -1` 형태여서 **파이프라인 종료 코드가 tail의 것으로 대체**되어 lint 실패(exit 1)가 가려짐 — 실제로는 로컬에서도 실패 중이었음.

**조치**

- eslint.config.js에 `tools/**/*.mjs` 블록 추가(process/console readonly 전역). lint를 파이프 없이 재실행해 종료 코드 0을 직접 확인 후 재커밋.

**재발 방지**

- 성공 여부를 판정하는 검증 명령에서 출력 축약 파이프(`| tail`, `| grep`)를 종료 코드 확인과 함께 쓰지 않는다. 축약이 필요하면 `cmd; echo $?` 또는 명령을 분리한다.
