...
특정 v2 채널·정렬을 지정하려면 다음과 같이 실행합니다.
...
> 0.3.0부터 옵션 없는 `ains trends`는 v2 overview 브리핑(Official·Repos·Community·Research)이 기본입니다. 이전 랭킹은 `--ranking legacy`로 계속 사용할 수 있으며 0.4.0에서 제거될 예정입니다.
...
- 기본 랭킹: `v2` (0.3.0부터, 승인 게이트 통과) / `--ranking legacy`는 0.4.0까지 유지됩니다.
